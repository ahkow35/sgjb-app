...
async function getExchangeRate() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL ? '' : 'http://localhost:3000'}/api/exchange-rate`, {
...
async function getPetrolPrices() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL ? '' : 'http://localhost:3000'}/api/petrol`, {
    next: { revalidate: 86400 },
  })
  if (!res.ok) throw new Error('Failed to load petrol prices')
  return res.json()
}
...
async function getProducts(q: string, category: string) {
  ...
  if (q.trim()) {
    builder = builder.textSearch('name', q.split(/\s+/).join(' & '), { config: 'english' })
  }
